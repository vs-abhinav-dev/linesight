    ...
    zone_centers = np.vstack(
        (
            ...
            * np.expand_dims(np.arange(
                config_copy.n_zone_centers_extrapolate_before_start_of_map, 0, -1), axis=1),
            zone_centers,
            zone_centers[-1]
            + np.expand_dims(zone_centers[-1] - zone_centers[-2], axis=0)
            * np.expand_dims(np.arange(1, 1 +
                             config_copy.n_zone_centers_extrapolate_after_end_of_map, 1), axis=1),
        )
    )
...
def precalculate_virtual_checkpoints_information(zone_centers):
    ...
    """
    zone_transitions = 0.5 * \
        (zone_centers[1:] + zone_centers[:-1])  # shape: (n_points - 1, 3)
    # shape: (n_points - 1, 3)
    delta_zone_transitions = zone_transitions[1:] - zone_transitions[:-1]
    distance_between_zone_transitions = np.linalg.norm(
        delta_zone_transitions, axis=1)  # shape: (n_points - 2, )
...
def gbx_to_raw_pos_list(gbx_path: Path):
    ...
    print(ghost.race_time,
          f"ghost has {len(ghost.records)} records and {len(ghost.control_entries)} control entries")
    print("Keeping", records_to_keep, "out of", len(ghost.records),
          "records for a race time of", ghost.race_time / 1000)

    raw_positions_list = []
    for r in ghost.records[:records_to_keep]:
        raw_positions_list.append(
            np.array([r.position.x, r.position.y, r.position.z]))
...
def densify_raw_pos_list_n_times(raw_pos_list: List[npt.NDArray], n: int):
    interpolation_function = make_interp_spline(
        x=range(0, n * len(raw_pos_list), n), y=raw_pos_list, k=1)
    return list(interpolation_function(range(0, n * len(raw_pos_list))))


def map_name_from_map_path(map_path):
    gbx = Gbx(str(config_copy.trackmania_base_path / "Tracks" /
              "Challenges" / Path(map_path.strip("'\""))))
...
def replay_personal_record(map_path):
    ...
    filename = trackmania_replay_username + "_" + \
        map_name_from_map_path(map_path) + ".Replay.gbx"
...
def get_checkpoint_positions_from_gbx(map_path: str):
    ...
    """
    g = Gbx(str(config_copy.trackmania_base_path / "Tracks" /
            "Challenges" / map_path.strip("'\"").replace("\\", "/")))

    challenges = g.get_classes_by_ids(
    ...
    for block in challenge.blocks:
        if "Checkpoint" in block.name or "Line" in block.name:
            checkpoint_positions.append(
                np.array(block.position.as_array(), dtype="float"))
            if "High" in block.name:  # Added for E03
                checkpoint_positions[-1] += np.array([0, 7 / 8, 0])
            # Added for "exceed my tech"
            elif block.name in ["StadiumRoadMainCheckpointRight", "StadiumRoadMainCheckpointLeft"]:
                checkpoint_positions[-1] += np.array([0, 5 / 8, 0])
    checkpoint_positions = np.array(
        checkpoint_positions) * np.array([32, 8, 32]) + np.array((16, 0, 16))
...
def sync_virtual_and_real_checkpoints(zone_centers: npt.NDArray, map_path: str):
    ...
    max_allowable_distance_to_real_checkpoint = 9999999 * \
        np.ones(len(zone_centers))
    if config_copy.sync_virtual_and_real_checkpoints:
        ...
        for checkpoint_position in checkpoint_positions:
            dist_vcp_cp = np.linalg.norm(
                zone_centers - checkpoint_position, axis=1)
...
def find_indices_of_positions_near_cut_position(pos_list: List[npt.NDArray], cut_position: npt.NDArray, margin: float):
    ...
    """
    indices = []
    dist_pos_cut_pos = np.linalg.norm(
        np.array(pos_list) - cut_position, axis=1)
...
def analyze_map_cycle(map_cycle):
    ...
    """
    set_all_maps = set(map(lambda x: x[0], (a for a in itertools.chain(
        *copy.deepcopy(config_copy.map_cycle)))))
    set_maps_trained = set(map(lambda x: x[0], filter(
        lambda x: x[4], (a for a in itertools.chain(*copy.deepcopy(map_cycle))))))
