...
target_python_link_path = Path(os.path.expanduser(
    "~")) / "Documents" / "TMInterface" / "Plugins" / "Python_Link.as"

# Typically path(os.path.expanduser("~")) / "Documents" / "TrackMania"
# If running on linux it is path(os.path.expanduser("~")) / "Documents" / "TmForever" , not sure
trackmania_base_path = Path(os.path.expanduser("~")) / \
    "Documents" / "TmForever"
...
windows_TMLoader_path = Path(os.path.expanduser(
